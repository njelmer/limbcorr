...
Version: 2.1  (Last updated March 2017)
...
Revision history:
    ...
    Mar 2017    Created tables for GOES-13 Imager 
                 limb correction coefficients (v2.1)
                Updates nje_limbcorr_v2.py --> nje_limbcorr_v2.1.py
...
Input:
 Required:
   ...
   sensor: 	string,		name of sensor/instrument
					Valid options: abi, ahi, avhrr, goes_imager, modis, seviri, viirs
...
Notes on intecalibration:
	...
	- GOES-16 ABI has not yet been intercalibrated with other sensors.  Since ABI 
        is very similar to AHI, AHI intercalibration values are used when ABI is specified.
...
def limbcorr(data, lats, vza, satellite, sensor, jday, bandorder, \
		...
		tblpath='./nje_limbcorr_v2.1/tables/', fillval=np.nan):
  ...
  valid = ['goes-13', 'goes-14', 'goes-15', 'goes-16', 'himawari', \
       'noaa-15', 'noaa-18', 'noaa-19', 'metop-a', 'metop-b', \
       'aqua', 'terra', 'meteosat', 'snpp']
  ...
  elif sensor == 'goes_imager'   : 
       satalt = 35800.  #km
       if satellite != 'goes-13' and satellite != 'goes-14' and satellite != 'goes-15':
          raise ValueError('Invalid satellite/sensor combination')
  elif sensor == 'ahi'   : 
       satalt = 35800.  #km
  ...
  szvza = np.shape(vza)
  angle_dims = len(szvza)
  if szvza != szl or szvza[0] != sz[0] or szvza[1] != sz[1] or angle_dims != 2: 
  ...
  for m in range(nbands):
    ...
    if sensor == 'viirs'  and (bandorder[m] < 12 or bandorder[m] > 16): 
       correct[:,:,m] = data[:,:,m]
       continue
    if sensor == 'goes_imager' and (bandorder[m] < 2 or bandorder[m] > 6 or bandorder[m] ==5): 
       correct[:,:,m] = data[:,:,m]
       continue
